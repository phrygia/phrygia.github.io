...
유효성을 검사하기는 하지만 회원가입 버튼을 누르면 자꾸 서버에 post요청을 보냈다. <br>
react 코드안에서 if {} else {} 구문에서 return 사용이 까다롭고 생각처럼 작동을 하지 않았다.<br>
고민하다 useState로 유효성 통과 여부를 체크하기로 하였다. <br>
setCheckError가 하나라도 false가 된다면 서버 요청이 되지 않는다.

```js
// useState 추가
const [checkError, setCheckError] = useState(false);
const [registerError, setRegisterError] = useState(''); // 회원가입 실패 에러
...
const handleSubmit = (e) => {
    ...
    if (checkError) onhandlePost(joinData);
}
```
...
const Register = () => {
  ...
  const onhandlePost = async (data) => {
    ...
    // post
    await axios.post('/member/join', postData)
